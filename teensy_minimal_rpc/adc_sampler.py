        ...
        **TODO** Should this be handled differently?
        ...
        if differential:
            if (resolution is not None) and ((resolution < 16) and not
                ...
                # differential mode. However, bit-widths are actually increased
                # by one bit, where the additional bit indicates the sign of
                # the result.
                bit_width += 1

        elif gain_power > 0:
            raise ValueError('Programmable gain amplification is only valid '
        ...
        # rate.
        # **TODO** The reasoning behind selecting the minimum conversion rate
        # is that we expect it to lead to the lowest noise possible while still
        # matching the specified requirements.
        ...
        if sampling_rate_hz is None:
            ...
            #
            # **TODO** We arrived at this after a handful of empirical tests.
            # We think this is necessary due to the slight deviation of the
            # computed conversion rates (see TODO in
        ...
        df_volts = reference_V * (df_adc_results / (1 << (resolution +
                                                            adc_settings
                                                            .gain_power)))
